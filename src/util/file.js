...
import { uniqueId, lower } from './string'
...
export class UploadControl {
  ...
  storageName (req, file, cb) {
    ...
    if (!lower(file.originalname).match(regex)) return cb(new Error('Tipo de arquivo não suportado!'))
    let name = file.originalname.split('.')
    this.file.type = `.${name.pop()}`
    this.file.name = name.join('.')
    this.file.hash = `${uniqueId(this.file.name)}`
    cb(null, `${this.file.hash}${this.file.type}`)
  }
